...
function mostrarAgregarFacturaCompra() {
    ...
    cargarListaDeposito("#deposito_lst");
    cargarListaTipoPago("#tipo_pago_lst");
    $("#sucursal_lst").val("1");
    //obtener ultimo id
...
function calcularTotalFacturaCompra() {
    ...
    $("#factura_compra tr").each(function (evt) {
        totalexenta += quitarDecimalesConvertir($(this).find("td:eq(5)").text());
        totaliva5 += quitarDecimalesConvertir($(this).find("td:eq(6)").text());
        totaliva10 += quitarDecimalesConvertir($(this).find("td:eq(7)").text());

        total += quitarDecimalesConvertir($(this).find("td:eq(5)").text()) +
                quitarDecimalesConvertir($(this).find("td:eq(6)").text()) +
                quitarDecimalesConvertir($(this).find("td:eq(7)").text());
    });
...
function guardarFacturaCompra() {
    ...
    let cabecera = {
        ...
        'cod_tipo_pago': $("#tipo_pago_lst").val(),
...
//guardamos el libro compraw
    let libro = {
        ...
        'total': Math.round(quitarDecimalesConvertir($("#total").text()) / 21),
        ...
        'grav10': Math.round(quitarDecimalesConvertir($("#total_iva10").text())) -
                Math.round(quitarDecimalesConvertir($("#total_iva10").text()) / 11),
        'exenta': Math.round(quitarDecimalesConvertir($("#total_exenta").text())),
    ...
    $("#factura_compra tr").each(function (evt) {
        let detalle = {
            ...
            'costo': quitarDecimalesConvertir($(this).find("td:eq(3)").text()),
            'exenta': quitarDecimalesConvertir($(this).find("td:eq(5)").text()),
            'iva5': quitarDecimalesConvertir($(this).find("td:eq(6)").text()),
            'iva10': quitarDecimalesConvertir($(this).find("td:eq(7)").text()),
...
function cargarTablaFacturaCompra() {
    ...
    if (data === "0") {
        ...
        json_data.map(function (item) {
            ...
            fila += `<td>
                        <button onclick="imprimirFacturaCompra(${item.cod_compra}); return false;" class='btn btn-warning imprimir-orden_compra'><i class='ti ti-printer'></i></button>
                        <button ${(item.estado === "CONFIRMADO" || item.estado === "ANULADO") ? "disabled" : ""} class='btn btn-danger  anular-factura_compra'><i class='ti ti-ban'></i></button>
                    </td>`;
