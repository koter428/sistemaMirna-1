...
function mostrarAgregarOrdenCompra() {
    ...
    if (ultimo === "0") {
        ...
        $("#cod").val(quitarDecimalesConvertir(json_ultimo['cod_orden_compra']) + 1);
...
// Si no se encontró material repetido, agrega una nueva fila
    if (!materialRepetido) {
        ...
        <tr>
            ...
            <td><input class="form-control costo-presu" readonly value="${formatearNumero($("#costo_txt").val())}"></td>
...
function cargarTablaOrdenCompra() {
    ...
    if (data === "0") {
        ...
        json_data.map(function (item) {
            ...
            fila += `<td>
                        <button ${(item.estado === "CONFIRMADO" || item.estado === "ANULADO") ? "disabled" : ""} class='btn btn-success  aprobar-orden_compra'><i class='ti ti-check'></i></button>
                        <button onclick="imprimirOrdenCompra(${item.cod_orden_compra}); return false;" class='btn btn-warning imprimir-orden_compra'><i class='ti ti-printer'></i></button>
                        <button ${(item.estado === "CONFIRMADO" || item.estado === "ANULADO") ? "disabled" : ""} class='btn btn-danger  anular-orden_compra'><i class='ti ti-ban'></i></button>
                    </td>`;
            fila += `</tr>`;
        });
    }
            ...
            json_data.map(function (item) {
                $("#orden_compra_compra").append(`
                    <tr>
                        ...
                        <td><input readonly class="form-control costo-presu" value="${formatearNumero(item.costo)}"></td>
                        <td>${item.cantidad}</td>
...
function cargarListaOrdenPendientes(componente) {
    let datos = ejecutarAjax("controladores/orden_compra.php", "Orden_pendientes=1");
    console.log(datos);
    let option = "<option value='0'>Selecciona una Orden</option>";
    if (datos !== "0") {
        let json_datos = JSON.parse(datos);
        json_datos.map(function (item) {
            option += `<option value='${item.cod_orden_compra}'>Nro de Orden ${item.cod_orden_compra} | ${item.nom_ape_prov}  | Total: ${formatearNumero(item.total)}</option>`;
        });
    }
...
$(document).on("click", ".anular-orden_compra", function (evt) {
     let id = $(this).closest("tr").find("td:eq(0)").text();
     if($(this).closest("tr").find("td:eq(5)").text() === "ANULADO"){
    ...
    Swal.fire({
        ...
        if (result.isConfirmed) {
            ...
            console.log(response);
            mensaje_confirmacion("Anulado Correctamente", "ANULADO");
            mostrarListarOrdenCompra();
        }
    });
});
...
//------------------------------------------------------------------------------
//------------------------------------------------------------------------------
$(document).on("click", ".aprobar-orden_compra", function (evt) {
     let id = $(this).closest("tr").find("td:eq(0)").text();
     if($(this).closest("tr").find("td:eq(5)").text() === "ANULADO"){
         mensaje_dialogo_info_ERROR("El registro ya ha sido anulado anteriormete", "ATENCION");
         return;
    ...
    Swal.fire({
        ...
        if (result.isConfirmed) {
            ...
            console.log(response);
            mensaje_confirmacion("Anulado Correctamente", "APROBADO");
            mostrarListarOrdenCompra();
        }
    });
});