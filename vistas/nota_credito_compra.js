    ...
    $("#nota_compra tr").each(function (evt) {
         if ($(this).find("input").filter(":eq(1)").prop("checked")) {
            ...
            let detalle = {
                ...
                'iva_5': quitarDecimalesConvertir($(this).find("td:eq(6)").text()),
                'iva_10': quitarDecimalesConvertir($(this).find("td:eq(7)").text()),
...
function cargarTablaNotaCreditoCompra() {
    ...
    if (data === "0") {
        ...
        json_data.map(function (item) {
            ...
            fila += `<td>
                        <button onclick="imprimirNotaCreditoCompra(${item.cod_nota_compra}); return false;" class='btn btn-warning imprimir-nota_credito'><i class='ti ti-printer'></i></button>
                        <button ${(item.estado === "CONFIRMADO" || item.estado === "ANULADO") ? "disabled" : ""} class='btn btn-danger  anular-nota_credito'><i class='ti ti-ban'></i></button>
                    </td>`;
...
$(document).on("change", ".cantidad-nota", function (evt) {
    ...
    let costo = quitarDecimalesConvertir($(this).closest("tr").find("td:eq(3)").text());
    let impuesto = $(this).closest("tr").find("td:eq(8)").text();
    ...
    if (impuesto === "10") {
        $(this).closest("tr").find("td:eq(7)").text(formatearNumero(costo * cantidad));
    }
    calcularTotalNotaCreditoCompra();
