...
function mostrarAgregarRemision() {
    ...
    cargarListaVehiculo("#vehiculo_lst");
    ...
    if (ultimo === "0") {
        ...
        $("#cod").val(quitarDecimalesConvertir(json_ultimo['cod_remision']) + 1);
...
function guardarRemision(){
    ...
    $("#ajuste_stock_compra tr").each(function(evt) {
        let detalle = {
            ...
            'cantidad_factura' : $(this).find("td:eq(3)").text(),
...
function cargarTablaRemision() {
    ...
    if (data === "0") {
        ...
        json_data.map(function (item) {
            ...
            fila += `<td>
                        <button onclick="imprimirRemision(${item.cod_nota_remision}); return false;" class='btn btn-warning imprimir-orden_compra'><i class='ti ti-printer'></i></button>
                        <button ${(item.estado === "CONFIRMADO" || item.estado === "ANULADO") ? "disabled" : ""} class='btn btn-danger  anular-remision'><i class='ti ti-ban'></i></button>
                    </td>`;
