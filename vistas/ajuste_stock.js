...
function mostrarAgregarAjusteStock() {
    ...
    if (ultimo === "0") {
        ...
        $("#cod").val(quitarDecimalesConvertir(json_ultimo['cod_ajuste_stock']) + 1);
    }
...
function guardarAjusteStock(){
    ...
    let cabecera = {
        ...
        'tipo_ajuste' : $("#tipo").val(),
        'fecha_ajuste' : $("#fecha").val(),
        'cod_compra' : $("#compra_lst").val(),
...
function cargarTablaAjusteStock() {
    ...
    if (data === "0") {
        ...
        json_data.map(function (item) {
            ...
            fila += `<td>
                        <button onclick="imprimirAjuste(${item.cod_ajuste_stock}); return false;" class='btn btn-warning imprimir-ajuste_stock'><i class='ti ti-printer'></i></button>
                        <button ${(item.estado === "CONFIRMADO" || item.estado === "ANULADO") ? "disabled" : ""} class='btn btn-danger  anular-ajuste_stock'><i class='ti ti-ban'></i></button>
                    </td>`;
